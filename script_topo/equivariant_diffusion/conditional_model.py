...
class ConditionalDDPM(EnVariationalDiffusion):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 添加拓扑结构相似性损失的权重系数
        self.topo_loss_weight = 20
        # 化学键截断距离
        self.bond_cutoff = 2.4
        # 软化参数
        self.softness = 0.1
    
    def compute_topology_similarity_loss(self, gen_coords, gen_features, gen_mask, 
                                       target_coords, target_features, target_mask):
        """
        简化且数值稳定的拓扑结构损失
        - 使用欧氏距离直接构建邻接矩阵
        - 采用安全的权重计算方式
        ...
        try:
            # 基本参数设置
            batch_size = max(torch.max(target_mask).item(), torch.max(gen_mask).item()) + 1
            total_loss = torch.tensor(0.0, device=target_coords.device)
            valid_batches = 0  # 有效批次计数
            
            for i in range(batch_size):
                batch_target_mask = (target_mask == i)
                batch_gen_mask = (gen_mask == i)
                
                n_target = batch_target_mask.sum().item()
                n_gen = batch_gen_mask.sum().item()
                
                # 只处理有足够原子且数量匹配的批次
                if n_target > 2 and n_gen > 2 and n_target == n_gen:
                    try:
                        # 提取坐标
                        target_coords_batch = target_coords[batch_target_mask]
                        gen_coords_batch = gen_coords[batch_gen_mask]
                        
                        # 计算距离矩阵
                        adj_target = torch.cdist(target_coords_batch, target_coords_batch, p=2.0)
                        adj_gen = torch.cdist(gen_coords_batch, gen_coords_batch, p=2.0)
                        
                        # 排除数值异常
                        if torch.isnan(adj_target).any() or torch.isnan(adj_gen).any():
                            continue
                        
                        # 移除自连接
                        eye = torch.eye(n_target, device=adj_target.device)
                        
                        # 设置最小距离避免除零问题
                        min_dist = 0.2  # 安全的最小距离值
                        adj_target = torch.clamp(adj_target, min=min_dist) * (1 - eye)
                        adj_gen = torch.clamp(adj_gen, min=min_dist) * (1 - eye)
                        
                        # 计算距离差异
                        adj_diff = torch.abs(adj_target - adj_gen)
                        
                        # 安全的权重计算（不使用指数函数）
                        # 使用反比例函数：weight = α/(dist^β)
                        alpha = 1.0
                        beta = 2.0
                        
                        weight_target = (alpha / (adj_target ** beta))**2
                        weight_gen = (alpha / (adj_gen ** beta))**2
                        
                        # 取两个权重的平均
                        weight_matrix = 0.5 * (weight_target + weight_gen)
                        
                        # 合理限制权重范围
                        weight_matrix = torch.clamp(weight_matrix, 0.1, 3.0) * (1 - eye)
                        
                        # 计算加权差异
                        weighted_diff = adj_diff * weight_matrix
                        
                        # 安全的损失计算
                        mask = (1 - eye).bool()
                        if torch.sum(mask) > 0:  # 确保有有效元素
                            batch_loss = torch.sum(weighted_diff[mask]) / torch.sum(mask)
            ...
            if valid_batches > 0:
                ...
                if torch.isnan(avg_loss) or torch.isinf(avg_loss):
                    return torch.tensor(0.01, device=target_coords.device)
                return avg_loss
            else:
                return torch.tensor(0.01, device=target_coords.device)
    ...
    def sample_p_xh_given_z0(self, z0_lig, xh0_pocket, lig_mask, pocket_mask,
                             batch_size, prompt_labels, ref_ligand=None, fix_noise=False):
        """Samples x ~ p(x|z0)."""
        ...
        net_out_lig, _ = self.dynamics(
            z0_lig, xh0_pocket, t_zeros, lig_mask, pocket_mask, prompt_labels, ref_ligand=ref_ligand)
    ...
    def forward(self, ref_ligand, pocket, opt_ligand, prompt_labels, return_info=False):
        ...
        net_out_lig, _ = self.dynamics(
            z_t_lig, xhc0_pocket, t, opt_ligand['mask'], pocket['mask'], prompt_labels, ref_ligand=ref_ligand)
        ...
        # 计算拓扑结构相似性损失
        # 仅在t非零时计算拓扑结构损失，因为t=0时会直接计算log p(x|z0)
        # 使用opt_ligand作为目标拓扑结构（即我们希望生成的结构）
        topo_loss = torch.zeros_like(t_int.squeeze())
        if t_is_not_zero.sum() > 0:
            # 计算未噪声化的分子结构
            x_lig_hat = xh_lig_hat[:, :self.n_dims]
            h_lig_hat = F.softmax(xh_lig_hat[:, self.n_dims:], dim=1)  # 转换为概率分布
            
            # 计算拓扑相似性损失 - 使用opt_ligand作为目标拓扑结构
            batch_topo_loss = self.compute_topology_similarity_loss(
                opt_ligand['x'],            # 目标分子坐标
                opt_ligand['one_hot'],      # 目标分子特征
                opt_ligand['mask'],         # 目标分子掩码
                x_lig_hat,                  # 生成分子坐标
                h_lig_hat,                  # 生成分子特征
                opt_ligand['mask']          # 生成分子掩码
            )
            
            # 应用t不为零的掩码
            topo_loss = batch_topo_loss * t_is_not_zero.squeeze() * self.topo_loss_weight
        ...
        if self.training:
            ...
            net_out_0_lig, _ = self.dynamics(
                z_0_lig, xhc0_pocket, t_zeros, opt_ligand['mask'], pocket['mask'], prompt_labels, ref_ligand= ref_ligand)
        ...
        info = {
            ...
            'eps_hat_lig_h': scatter_mean(
                net_out_lig[:, self.n_dims:].abs().mean(1), opt_ligand['mask'],
                dim=0).mean(),
            'topo_loss': topo_loss.mean().item(),
        }
        
        # 将拓扑结构损失添加到loss_terms中
        loss_terms = (delta_log_px, error_t_lig, torch.tensor(0.0), topo_loss, SNR_weight,
    ...
    def diversify(self, ligand, pocket, noising_steps, prompt_labels, ref_ligand=None):
        """
        ...
        for s in reversed(range(0, noising_steps)):
            ...
            z_lig, xh_pocket = self.sample_p_zs_given_zt(
                ...
                lig_mask, pocket['mask'], prompt_labels, ref_ligand=ref_ligand)

        # Finally sample p(x, h | z_0).
        x_lig, h_lig, x_pocket, h_pocket = self.sample_p_xh_given_z0(
            z_lig, xh_pocket, lig_mask, pocket['mask'], n_samples, prompt_labels, ref_ligand=ref_ligand)
    ...
    def sample_p_zs_given_zt(self, s, t, zt_lig, xh0_pocket, ligand_mask,
                             pocket_mask, prompt_labels, ref_ligand=None, fix_noise=False):
        ...
        eps_t_lig, _ = self.dynamics(
            zt_lig, xh0_pocket, t, ligand_mask, pocket_mask, prompt_labels, ref_ligand=ref_ligand)
    ...
    @torch.no_grad()
    def inpaint(self, ligand, pocket, prompt_labels, lig_fixed, ref_ligand=None, resamplings=1, return_frames=1,
        ...
        for s in reversed(range(0, timesteps)):
            ...
            for u in range(resamplings):
                ...
                z_lig_unknown, xh_pocket = self.sample_p_zs_given_zt(
                    ...
                    pocket['mask'], prompt_labels, ref_ligand=ref_ligand)
        ...
        x_lig, h_lig, x_pocket, h_pocket = self.sample_p_xh_given_z0(
            ...
            prompt_labels, ref_ligand=ref_ligand)
