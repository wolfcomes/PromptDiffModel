...
def get_ligand_and_pocket(data,virtual_nodes):
    ref_ligand = {
        ...
        'ref_ligand_bonds': data['ref_lig_bonds'].to('cuda', INT_TYPE)
    }
    if virtual_nodes:
...
device = 'cuda'
#checkpoint = torch.load('../checkpoints/topo/cddpm_epoch_16.pth')
checkpoint = torch.load('../checkpoints/topo/cddpm_ligand_only_topo_epoch_24.pth')
#checkpoint = torch.load('../checkpoints/zinc/cddpm_topo_epoch_42.pth')
# checkpoint = torch.load('../checkpoints/topo/zinc_epoch_18.pth')
