...
num_epochs = 50
device = 'cuda'
save_dir = '../checkpoints/topo'  # 模型保存的文件夹路径
loss_type = 'l2'
...
# 添加恢复训练参数
resume_epoch = 18  # 设为None表示从头开始，设为具体数字恢复指定epoch
...
for epoch in range(num_epochs):
    ...
    for batch in pbar:
        ...
        # 计算损失，返回 (nll, info)
        delta_log_px, error_t_lig, error_t_pocket, topo_loss, SNR_weight, \
        ...
        if loss_type == 'l2':
            ...
            topo_denom = actual_ligand_size  # 或其他更合适的归一化因子
            topo_loss = topo_loss / topo_denom

        nll = loss_t + loss_0 + kl_prior + topo_loss
    ...
    # 每个 epoch 后保存模型
    torch.save(checkpoint, os.path.join(save_dir, f'cddpm_ligand_only_topo_epoch_{epoch}.pth'))
