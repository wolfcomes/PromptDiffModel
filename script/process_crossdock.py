...
def saveall(filename, pdb_and_mol_ids, lig_coords, lig_one_hot,lig_bonds, lig_mask,
            pocket_coords, pocket_one_hot, pocket_mask, prompt_labels):
    np.savez(filename,
...
if __name__ == '__main__':
    ...
    datadir = '../data/docking_results/group_1/'
    ...
    for split in data_split.keys():
        ...
        for (pocket_fn, ligand_fn) in pbar:
            ...
            lig_bond.append(ligand_data['lig_bonds'])
        ...
        print(len(lig_coords))
        print(len(lig_one_hot))
        lig_coords = np.concatenate(lig_coords, axis=0)
        lig_one_hot = np.concatenate(lig_one_hot, axis=0)
        prompt_labels = np.tile([0, 0, 1], (len(lig_coords), 1))
        lig_bond = np.concatenate(lig_bond, axis=0)
        ...
        saveall(processed_dir / f'{split}.npz', pdb_and_mol_ids, lig_coords,
                lig_one_hot,lig_bond, lig_mask, pocket_coords,
                pocket_one_hot, pocket_mask, prompt_labels)
