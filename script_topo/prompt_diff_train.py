...
# 添加恢复训练参数
resume_epoch = 6  # 设为None表示从头开始，设为具体数字恢复指定epoch
resume_checkpoint_path = os.path.join(save_dir, f'cddpm_ligand_only_topo_epoch_{resume_epoch}.pth') if resume_epoch is not None else None
# 加载检查点（如果存在）
start_epoch = 0
...
for epoch in range(num_epochs):
    ...
    for batch in pbar:
        ...
        if loss_type == 'l2':
            ...
            topo_loss = topo_loss / actual_ligand_size
    ...
    # 每个 epoch 后保存模型
    torch.save(checkpoint, os.path.join(save_dir, f'cddpm_topo_epoch_{epoch}.pth'))
